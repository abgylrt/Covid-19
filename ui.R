...
ui <- fluidPage(theme = shinytheme("yeti"),
  ...
  tags$style(HTML(".col-sm-10 { padding: 5px; margin-bottom: -15px; }")),
  tags$style(HTML(".col-sm-5 { padding: 5px; margin-bottom: -15px; }")),
  navbarPage(title = div("COVID-19 Dashboard", style = "padding-left: 10px"),
    ...
    navbarMenu("More", icon = icon("question-circle"),
               tabPanel(tags$a("", href = "https://github.com/abgylrt/Covid-19", target = "_blank",list(icon("github"), "Resources"))),
               tabPanel(tags$a("", href = "https://www.linkedin.com/in/abigayle-rachman-b83a76176/", target = "_blank",list(icon("linkedin"), "My Profile"))),
    ...
    header.append('<div style=\"float:right\"><a target=\"_blank\" href=\"https://www.youtube.com/watch?v=cNcLxXbFEEA\"><img src=\"Abigayle.jpg\" alt=\"alt\" style=\"float:right;width:33px;padding-top:10px;margin-top:-50px;margin-right:10px\"> </a></div>');
    console.log(header)")
    )
  )
)